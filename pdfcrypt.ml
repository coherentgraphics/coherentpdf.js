        ...
        let k1_64 = String.concat "" (many k1 64) in
          ...
          in
            ...
            k := (match mod3 e with
                    ...
                    | _ -> (*flprint "using sha512\n";*) sha512) (Pdfio.input_of_string e);
            (*flprint "New k is:\n";
            prs !k;
  ...
  if String.length o < 48 || String.length u < 48 then raise (Pdf.PDFError "/O too short in make_intermediate_owner_key_aesv3") else
    let d =
      ...
      | None ->
          let i =
            int_array_of_string
            ((if iso then shamix utf8pw (Some u) else sha256)
...
let file_encryption_key_aesv3_user iso utf8pw u ue =
  if String.length u < 48 then raise (Pdf.PDFError "/U too short in file_encryption_key_aesv3_user") else
    aes_decrypt_data ~remove_padding:false
      8
      (int_array_of_string ((if iso then shamix utf8pw None else sha256) (Pdfio.input_of_string (String.concat "" [utf8pw; String.sub u 40 8]))))
...
let authenticate_owner_password_aesv3 iso utf8pw u o =
  if String.length o < 48 || String.length u < 48 then raise (Pdf.PDFError "/O too short in authenticate_owner_password") else
    (if iso then shamix utf8pw (Some u) else sha256) (Pdfio.input_of_string (String.concat "" [utf8pw; String.sub o 32 8; String.sub u 0 48])) = String.sub o 0 32
...
let authenticate_user_password_aesv3 iso utf8pw u =
  if String.length u < 48 then raise (Pdf.PDFError "/U too short in authenticate_owner_password") else
    (if iso then shamix utf8pw None else sha256) (Pdfio.input_of_string (String.concat "" [utf8pw; String.sub u 32 8])) = String.sub u 0 32
...
(* At some point, we changed it so that /Encrypt is indirect. Unfortunately,
this breaks linearized encrypted writing - both processes depend on objects not
being renumbered. Changed it back now. Why did we change it in the first place?
Did some old versions of Acrobat or some other viewer need an indirect
/Encrypt? *)
...
let make_ue iso file_encryption_key user_pw user_validation_salt user_key_salt =
  let hash =
    if iso then shamix user_pw None else sha256
  in
  let u =
    String.concat
      ""
...
let make_oe iso file_encryption_key owner_pw owner_validation_salt owner_key_salt u =
  let hash =
    if iso then shamix owner_pw (Some u) else sha256
  in
    let o =
      String.concat
       ""
